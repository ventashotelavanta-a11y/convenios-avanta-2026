...
def crear_convenio_pdf(numero_convenio, cliente, fecha, tarifas=None, año_vigencia=None):
    ...
    # ========== TEXTO INTRODUCTORIO (según plantilla: dos líneas separadas) ==========
    c.setFont('Helvetica', 12)
    c.drawString(60, y, 'Por parte de Avanta Hotel & Villas.')
    y -= 15
    
    texto_intro_2 = f"Encontrará las tarifas especiales para {cliente.get('empresa', 'Empresa')}"
    ...
    # Especificación con negritas selectivas según plantilla
    c.setFont('Helvetica-Bold', 12)
    c.drawString(80, y, 'La tarifa convenio está disponible únicamente para reservaciones realizadas')
    y -= 15
    c.drawString(80, y, 'directamente con el hotel, a través de nuestro motor de reservaciones con')
    y -= 15
    c.drawString(80, y, 'el código de Promocional: AVANTA')
    y -= 15
    
    # Año SIN paréntesis - Según plantilla
    c.setFont('Helvetica', 12)
    ...
    c.setFont('Helvetica', 12)
    c.drawString(x_pos, y, 'con las respectivas')
    y -= 15
    
    c.drawString(80, y, 'actualizaciones de tarifa y documento')
    y -= 15
    
    c.drawString(80, y, 'Todas las reservaciones están sujetas a disponibilidad.')
    y -= 30
    
    # ========== DESPEDIDA (según plantilla) ==========
    c.setFont('Helvetica', 12)
    c.drawString(60, y, 'Agradezco su atención.')
    y -= 60
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        response = {
            'status': 'active',
            'version': 'Plantilla 2026 DEFINITIVA',
